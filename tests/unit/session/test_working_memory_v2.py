...
def test_dynamic_reminders_flag_large_key_facts_and_bulk_urls():
    ...
    assert "DYNAMIC SECTION SIZE REMINDERS" in reminders
    assert "Key Facts & Decisions has 41 list items" in reminders
    assert "Files & Context has 22 list items and 22 URL-like entries" in reminders
    assert "directly relevant" in reminders
...
def test_key_facts_allows_safe_consolidation_when_oversized():
    ...
    assert "- Decision 1: keep module_1.py because API contract 1 is stable." not in merged
...
def test_files_context_can_prune_bulk_urls_while_preserving_file_paths():
    old_wm = _wm(
        files_context=(
    ...
    assert "unused.png" not in merged
