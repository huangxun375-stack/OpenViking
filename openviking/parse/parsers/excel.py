...
def _env_int(name: str, default: int) -> int:
    ...
    try:
        ...
        logger.warning(f"[ExcelParserProcessPool] Invalid int env {name}={raw!r}; using {default}")
        return default
...
def _env_float(name: str, default: float) -> float:
    ...
    try:
        ...
        logger.warning(f"[ExcelParserProcessPool] Invalid float env {name}={raw!r}; using {default}")
        return default


def _get_excel_layout_executor() -> concurrent.futures.ProcessPoolExecutor:
    global _EXCEL_LAYOUT_EXECUTOR
    if _EXCEL_LAYOUT_EXECUTOR is None:
        workers = max(1, _env_int("OPENVIKING_EXCEL_PARSE_PROCESS_WORKERS", 2))
        start_method = os.environ.get("OPENVIKING_EXCEL_PARSE_PROCESS_START_METHOD", "spawn")
        _EXCEL_LAYOUT_EXECUTOR = concurrent.futures.ProcessPoolExecutor(
            max_workers=workers,
            mp_context=get_context(start_method),
        )
        logger.info(
            f"[ExcelParserProcessPool] Started process pool workers={workers} "
...
class ExcelParser(BaseParser):
    ...
    def _should_use_process_pool(self, path: Path, kwargs: Dict[str, Any]) -> bool:
        if not _env_bool("OPENVIKING_EXCEL_PARSE_PROCESS_POOL", False):
            return False
        if path.suffix.lower() == ".xls":
            return False
        if kwargs.get("enable_link_rewrite") or kwargs.get("base_dir") or kwargs.get("allowed_media_dirs"):
            ...
            return False
        min_chars = _env_int("OPENVIKING_EXCEL_PARSE_PROCESS_MIN_CHARS", 200_000)
        if min_chars <= 0:
            return True
        try:
            return path.stat().st_size >= min_chars
        except OSError:
            return False
    ...
    async def _parse_existing_path_process_pool(
        ...
        timeout_s = _env_float("OPENVIKING_EXCEL_PARSE_PROCESS_TIMEOUT", 120.0)
        future = loop.run_in_executor(
            _get_excel_layout_executor(),
            partial(
        ...
        worker_result = await asyncio.wait_for(future, timeout=timeout_s)
